...
const getChapterTitle = (item: ManifestItem | TocElement, index: number): string => {
...
async function loadEpub(filePath: string, timeoutMs = 15000): Promise<EPub> {
  ...
  await Promise.race([
    ...
    new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error(`EPUB parse timed out after ${timeoutMs}ms`)), timeoutMs)
    ),
  ]);
  return epub;
}
...
export const getFile = async (
  ...
  try {
    ...
    const chapterSource: Array<ManifestItem | TocElement> = useFlow ? epub.flow : epub.toc;

    let toc: TocItem[] = [];
    if (chapterSource && chapterSource.length > 0) {
      console.log(`[getFile] Using ${useFlow ? 'epub.flow' : 'epub.toc'} for TOC mapping.`);
      ...
      console.warn(`[getFile] Both epub.flow and epub.toc are empty for ${filename}.`);
    }
...
const getEpubMetadata = async (
  ...
  try {
    ...
    console.error(`[getEpubMetadata] Error parsing metadata for ${filename}: ${message}`);
    return null;
  }
};
...
export const getBooks = async (_req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const validBooksData = booksData.filter((book): book is BookSummary => book !== null);
...
const loadBookJsonStructure = async (filePath: string): Promise<BookJsonData> => {
  ...
  const useFlow = epub.flow && epub.flow.length > 0;
  const chapterList: Array<ManifestItem | TocElement> = useFlow ? epub.flow : epub.toc || [];
  console.log(
    `[loadBookJsonStructure] EPUB metadata parsed. ${useFlow ? 'flow' : 'toc'} length: ${chapterList.length}`
  );

  if (chapterList.length === 0) {
    console.warn('[loadBookJsonStructure] EPUB flow/toc is empty. No chapters to include.');
    return bookJson;
  }

  const chapterPromises: Promise<BookChapter>[] = chapterList.map(async (chapterRef, index) => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      console.error(`[loadBookJsonStructure] Error getting chapter ID ${chapterRef.id}: ${message}`);
      return {
        id: chapterRef.id,
        title: fallbackTitle.replace(/^Chapter (\d+)/, 'Chapter $1 (Error Loading)'),
        content: null,
        error: message,
      };
    }
...
export const handleChatQuery = async (
  ...
  console.log(`[handleChatQuery] currentChapterIndex: ${currentChapterIndex}, provider: ${provider}`);
  ...
  try {
    ...
    console.log(`[handleChatQuery] Loaded book JSON. Total chapters: ${bookJsonData.chapters.length}`);
    ...
    const aiResponse = await service.getChatResponse(trimmedBookJson, context, query);
    ...
    const errorMessage = error instanceof Error ? error.message : 'Failed to process chat query';
