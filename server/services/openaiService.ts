...
class OpenAIService {
  async getChatResponse(
    ...
    try {
      ...
      console.error(
        "[openaiService.getChatResponse] Error stringifying book JSON:",
        stringifyError,
      );
...
- Otherwise, be a thoughtful, concise companion: discuss what's actually on the page, reference chapter titles or IDs where helpful, and engage with the user's interpretation.${truncationMessage}`;
      ...
      console.log(
        "[openaiService.getChatResponse] Sending request to OpenAI API (/v1/responses)...",
      );
      ...
      console.log(
        "[openaiService.getChatResponse] OpenAI API Response received.",
      );
      ...
      if (!responseContent) {
        console.error(
          "[openaiService.getChatResponse] OpenAI response missing output text.",
        );
      ...
      console.error(
        "[openaiService.getChatResponse] Error calling OpenAI API for chat:",
        error,
      );
  ...
  async *getChatResponseStream(
    ...
    console.log(
      `[openaiService.stream] Chapters: ${bookJsonData.chapters?.length || 0}; query: "${query}"`,
    );
    ...
    try {
      ...
      console.error("[openaiService.stream] Error stringifying:", stringifyError);
      throw new Error("Failed to prepare book data for AI.");
    }
...
- Otherwise, be a thoughtful, concise companion: discuss what's actually on the page, reference chapter titles or IDs where helpful, and engage with the user's interpretation.${truncationMessage}`;
    ...
    for await (const event of stream) {
      // The Responses API emits various event types; we only care about
      // text deltas being appended to the model's output.
